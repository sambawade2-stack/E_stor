<x-admin-layout :title="'Client : '.$customer->name">
    ...
    <div class="mb-6 grid gap-4 sm:grid-cols-3">
        ...
        <div class="rounded-2xl border border-gray-100 bg-white p-5">
            ...
            <p class="mt-1 text-3xl font-extrabold">{{ $customer->orders_count }}</p>
        </div>
        <div class="rounded-2xl border border-gray-100 bg-white p-5">
            <p class="text-sm font-semibold uppercase tracking-wide text-gray-500">Total dépensé</p>
            <p class="mt-1 text-3xl font-extrabold">{{ format_price($customer->total_spent ?? 0) }}</p>
            <p class="mt-1 text-sm text-gray-500">hors commandes annulées</p>
        </div>
    </div>

    <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white">
        <h2 class="border-b border-gray-100 px-6 py-4 text-lg font-bold">Historique des commandes</h2>
